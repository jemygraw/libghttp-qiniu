...
int qn_upload_file(const char *local_path, const char *upload_token, const char *file_key, qn_putextra *put_extra, qn_putret *put_ret)
{
    ...
    if (file_key != NULL)
    {
        form_data_p = qn_addformfield(form_data_p, form_boundary, form_boundary_len, "key", (char *)file_key,
                                      strlen(file_key), NULL, &form_data_len);
    }
    ...
    file_body = (char *)malloc(sizeof(char) * file_len);
    ...
    form_data_p = qn_addformfield(form_data_p, form_boundary, form_boundary_len, "file", file_body, file_len, mime_type,
                                  &form_data_len);
    free(file_body);
    file_body = NULL;
